...
def logerror(f, message, *args, **kwargs):
    errorfile = kwargs.get('errorfile', None) # allows for the use of a custom error log file
    ...
    if errorfile:
        ...
        if logdir:
            if os.path.isdir(logdir):
                ...
                print('Error, logdir does not exist. Error log will be in: {}'.format(dirname))
        errorfile = os.path.join(dirname,'{}_errors.csv'.format(os.path.basename(dirname))) # errors aggregated at the order level
...
class Api(object):
    ...
    def api_request(self, endpoint, data=None):
        ...
        if sys.version_info[0] < 3:
            ...
            # Warning, encodestring is deprecated in Python 3 but still works
            base64string = (base64
                            .encodestring(('%s:%s' % (self.username, self.password)).encode()).decode()
                            .replace('\n', ''))
...
class LocalStorage(object):
    ...
    def is_stored(self, scene): 
        if parsed_args.archive_directory:
            if os.path.exists(os.path.join(parsed_args.archive_directory,scene.filename)):
                return True
    ...
    def store(self, scene):
        ...
        while first_byte < file_size:
            ...
            try:
                ...
                print(str(e))
                logerror(self.tmp_scene_path(scene), str(e))
                break
...
def main(username, email, order, target_directory, password=None, host=None, verbose=False, archive_directory = None):
    if not password:
...
if __name__ == '__main__':
    ...
    parser.add_argument("-a", "--archive_directory",
                        required = False,
                        help = "path where downloaded scenes are archived")
